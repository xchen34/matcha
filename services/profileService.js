...
class ProfileService {
    ...
    const [profileResult, tagsResult, photosResult] = await Promise.all([
      pool.query(
        ...
        SELECT
          ...
          p.gender, p.sexual_preference, p.biography, p.birth_date, p.city, p.neighborhood, p.gps_consent, p.latitude, p.longitude,
          GREATEST(
              LEAST(
  ...
  async getPublicProfile(requestedId, currentUserId) {
    ...
    if (currentUserId) {
      ...
      promises.push(Promise.resolve({ rows: [{ i_liked: false, liked_me: false, reported_fake_by_me: false, blocked_by_you: false, blocked_you: false }] }));
    }

    const [profileResult, tagsResult, photosResult, relationResult] = await Promise.all(promises);
  ...
  async updateProfile(userId, data, tagsArray, photosArray) {
    ...
    try {
      ...
      const updatedProfile = await client.query(
        ...
        RETURNING user_id, biography, gender, sexual_preference, city, neighborhood, gps_consent, birth_date, latitude, longitude, fame_rating
        `,
        [
          userId, data.gender, data.sexual_preference, data.biography, data.birth_date,
          data.city, data.neighborhood, data.gps_consent, data.latitude, data.longitude
        ]
      );

      if (tagsArray !== null) {
        await client.query("DELETE FROM user_profile_tags WHERE user_id = $1", [userId]);
        if (tagsArray.length > 0) {
          const insertTagsQuery = tagsArray.map((_, i) => `($${i + 1})`).join(", ");
          await client.query(`INSERT INTO tags (name) VALUES ${insertTagsQuery} ON CONFLICT (name) DO NOTHING`, tagsArray);
          const tagIdsResult = await client.query(`SELECT id FROM tags WHERE name = ANY($1::text[])`, [tagsArray]);
          const tagIds = tagIdsResult.rows.map(row => row.id);
          if (tagIds.length > 0) {
             const userProfileTagsQuery = tagIds.map((_, i) => `($1, $${i + 2})`).join(", ");
             await client.query(`INSERT INTO user_profile_tags (user_id, tag_id) VALUES ${userProfileTagsQuery} ON CONFLICT DO NOTHING`, [userId, ...tagIds]);
          }
        }
      }
      ...
      if (photosArray !== null) {
        ...
        for (let i = 0; i < photosArray.length; i++) {
          ...
          const result = await client.query(
            `INSERT INTO user_photos (user_id, data_url, is_primary) VALUES ($1, $2, $3) RETURNING id, data_url, is_primary`,
            [userId, photo.data_url, photo.is_primary]
          );
          photosResultData.push(result.rows[0]);
        }
      } else {
        const result = await client.query(`SELECT id, data_url, is_primary FROM user_photos WHERE user_id = $1 ORDER BY is_primary DESC, id ASC`, [userId]);
        photosResultData = result.rows;
      }

      const userResult = await client.query(`SELECT email, username, first_name, last_name, email_verified, created_at FROM users WHERE id = $1`, [userId]);

      const tagsResult = await client.query(
        `SELECT t.name FROM user_profile_tags upt JOIN tags t ON t.id = upt.tag_id WHERE upt.user_id = $1 ORDER BY t.name ASC`,
        [userId]
      );
