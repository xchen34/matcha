...
new_users AS (
  ...
  SELECT
    lower(f || '.' || l || '.' || gen.g || '@example.com') AS email,
    lower(f || '_' || l || '_' || gen.g)                  AS username,
    f AS first_name,
    l AS last_name,
...
SELECT
  ...
  (ARRAY['Paris','Lyon','Marseille','Berlin','Madrid','London','New York','San Francisco'])[1 + floor(random() * 8)],
